---
title: Porządkowanie wiadomości e-mail w programie Outlook w sieci web
ms.author: daeite
author: daeite
manager: joallard
ms.date: 6/14/2019
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: f863adb782abac77c45336ad68a99006e070ee09
ms.sourcegitcommit: 1d98db8acb9959aba3b5e308a567ade6b62da56c
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 08/22/2019
ms.locfileid: "36556755"
---
# <a name="organize-your-email"></a>Porządkowanie wiadomości e-mail

W programie Outlook w sieci web umożliwia poleceń, takich jak **Czyszczenie** i **przenieść do** automatyczne usuwanie lub przenoszenie wiadomości od określonego nadawcy. Polecenia te pojawiają się nad listą wiadomości, po wybraniu wiadomości w skrzynce odbiorczej.

- **Sweep** <img src='data:image/png;base64,' />
 Umożliwia szybkie usuwanie wiadomości w skrzynce odbiorczej. Można automatycznie usunąć wszystkie przychodzące wiadomości e-mail od tego nadawcy, Zachowaj tylko ostatnią wiadomość e-mail i usunąć pozostałe lub usuń wiadomość e-mail starsze niż 10 dni.
- **Przenieś do** <img src='data:image/png;base64,' /> Umożliwia przenoszenie wiadomości określonej wiadomości do innego folderu.

Aby dowiedzieć się więcej, zobacz [Praca z folderów wiadomości w programie Outlook w sieci web](https://support.office.com/article/ae0f10d6-54e7-4f29-acd3-78cdc3fdcb9f).